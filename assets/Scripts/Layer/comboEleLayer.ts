...
@ccclass('comboEleLayer')
export class comboEleLayer extends layerControler {
    ...
    onClickButton(){
        ...
        // if(!this.isGive){
            gameInitManager.getDiamondManager().subDiamond(10);
            let node=find("Canvas/GameMain");
            let controlNode=node.getComponent(GameMain);
            controlNode.showAddDiamondInUnlockLayer(10);
        // }
        let anim=this.node.getChildByName("Mid").getChildByName("anim");
